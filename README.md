### Aron Lee

CTO, [INTFRAME](https://intframe.com).

Nine ventures. 53 production services, 45 sites, 18 databases, all on our own hardware — Seoul, Nha Trang, Singapore, Falkenstein.

[70.8B tokens, $91.3K, 104 days](https://www.viberank.app/profile/aron-intframe) through Claude Code and Codex CLI. Verified server-side, not self-reported.

### Open source

| | |
|---|---|
| [thermal-receipt](https://github.com/intframe/thermal-receipt) | Scroll-driven thermal receipt printer UI for React &nbsp;·&nbsp; [`@intframe/thermal-receipt`](https://www.npmjs.com/package/@intframe/thermal-receipt) |
| [scroll-rig](https://github.com/intframe/scroll-rig) | Scroll-driven three.js chapter patterns — camera beats, staggered assembly, adaptive quality governor |
| [scroll-qa](https://github.com/intframe/scroll-qa) | Playwright toolkit for QA-ing scroll- and animation-heavy sites |

### Elsewhere

[intframe.com/blog](https://intframe.com/blog) &nbsp;·&nbsp; [hello@intframe.com](mailto:hello@intframe.com)
